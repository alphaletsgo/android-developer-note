...
图中的”close”和“exit”是不同的，close只是调用`finish()`退出当前的Activity，但是Service并没有关掉，音乐会继续播放。而exit就是调用了`stopService(intent);`来停止服务，Service会调用`onDestroy()`方法来对mediaPlayer进行停止和释放资源。
